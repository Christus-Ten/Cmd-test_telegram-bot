...
function getDatabase(dbName) {
  return JSON.parse(fs.readFileSync(getDatabasePath(dbName), 'utf8'));
}

function saveDatabase(dbName, data) {
  fs.writeFileSync(getDatabasePath(dbName), JSON.stringify(data, null, 2));
}
...
const nix = {
  ...
  role: 0,
  cooldown: 0,
  guide: "{p}bank help - Affiche l'aide",
...
📈 Investing Skill: ${fonts.bold(bank.skills?.investing || 0)}`;
  ...
  deposit: async function (message, args, userData, bank, fonts, userId, balances, saveBalances, saveBank) {
...
💡 Your money is now earning interest in the bank!
  ...
  withdraw: async function (message, args, userData, bank, fonts, userId, balances, saveBalances, saveBank) {
    const amount = parseInt(args[1]);
    if (!amount || amount <= 0) {
      return message.reply(fonts.bold(`
💸 WITHDRAWAL HELP
...
Consider keeping funds in your vault for security.
  ...
  transfer: async function (message, args, userData, bank, fonts, userId, balances, saveBalances, saveBank, bot, chatId, msg) {
    ...
    if (msg.entities) {
      for (const entity of msg.entities) {
        if (entity.type === "mention") {
          ...
          // On pourrait résoudre le nom d'utilisateur, mais ici on utilise l'ID via une mention text_mention
        } else if (entity.type === "text_mention") {
          targetUID = entity.user.id;
          break;
        }
      }
    }
    // Fallback : si pas de mention, on cherche le premier mot après la commande
    if (!targetUID && args.length >= 3) {
      // On suppose que le deuxième argument est un @username, on peut essayer de le résoudre via bot.getChatMember
      // Pour simplifier, on va demander à l'utilisateur de mentionner correctement
    }
    ...
    try {
      // Charger les données bancaires du destinataire
      let targetBalances = getDatabase('balances');
      if (!targetBalances[targetUID]) targetBalances[targetUID] = { money: 0 };
      let targetBankData = getDatabase('bank');
      if (!targetBankData[targetUID]) targetBankData[targetUID] = createDefaultBank(targetUID);

      // Effectuer le transfert
      bank.balance -= amount;
      targetBankData[targetUID].balance += amount;
      ...
      targetBankData[targetUID].transactions.push({
        type: "transfer_in",
        amount: amount,
        date: Date.now(),
      ...
      balances[userId] = userData;
      saveBalances();
      let bankDb = getDatabase('bank');
      bankDb[userId] = bank;
      bankDb[targetUID] = targetBankData[targetUID];
      saveDatabase('bank', bankDb);

      // Obtenir le nom du destinataire
      let targetName = `User ${targetUID}`;
      try {
        const member = await bot.getChatMember(chatId, targetUID);
  ...
  loan: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
...
Example: bank loan 50000
  ...
  repayLoan: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
...
Example: bank repay ${Math.min(bank.loan, bank.balance)}
  ...
  savings: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
...
Example: bank savings 10000
  ...
  calculateInterest: async function (message, bank, fonts, userId, saveBank) {
    const now = Date.now();
...
`;
  ...
  collectInterest: async function (message, bank, fonts, userId, saveBank) {
    const now = Date.now();
...
`;
  ...
  freezeAccount: async function (message, bank, fonts, userId, saveBank) {
  ...
  dailyReward: async function (message, bank, fonts, userId, saveBank) {
    const now = Date.now();
...
Keep your streak alive for bigger rewards!
  ...
  work: async function (message, bank, fonts, userId, saveBank) {
    const now = Date.now();
...
Example: bank stocks list
  ...
  stocks: async function (message, args, bank, fonts, userId, saveBank) {
    ...
    if (!action || action === "list") {
      ...
      if (Object.keys(bank.stocks).length === 0) {
        ...
        Object.entries(bank.stocks).forEach(([symbol, shares]) => {
          const currentPrice = marketData.stocks[symbol]?.price || 0;
  ...
  crypto: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
    ...
    if (!action || action === "list") {
      ...
      if (Object.keys(bank.crypto).length === 0) {
        ...
        Object.entries(bank.crypto).forEach(([symbol, amount]) => {
          const currentPrice = marketData.crypto[symbol]?.price || 0;
  ...
  bonds: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();

    if (!action || action === "list") {
      let bondList = `${fonts.bold("🏛️ BOND MARKET")}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

      Object.entries(marketData.bonds).forEach(([type, data]) => {
    ...
    if (!bondType || !marketData.bonds[bondType]) {
    ...
    if (action === "sell") {
      ...
      if (!bank.bonds[bondType] || bank.bonds[bondType] < amount) {
      ...
      if (bank.bonds[bondType] === 0) {
        delete bank.bonds[bondType];
      }
...
`;
  ...
  collectDividend: async function (message, bank, fonts, userId, saveBank) {
  ...
  business: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
    ...
    if (action === "buy") {
      ...
      const businessData = marketData.businesses[businessType];
  ...
  shop: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
  ...
  realEstate: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();

    if (!action || action === "list") {
      let propertyList = `${fonts.bold("🏠 REAL ESTATE MARKET")}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

      Object.entries(marketData.properties).forEach(([type, data]) => {
  ...
  buyHouse: async function (message, args, bank, fonts, userId, saveBank) {
    return this.realEstate(message, args, bank, fonts, userId, saveBank);
  },

  rentProperty: async function (message, bank, fonts, userId, saveBank) {
    if (bank.realEstate.length === 0) {
  ...
  luxury: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();

    if (!action || action === "list") {
      let luxuryList = `${fonts.bold("💎 LUXURY COLLECTION")}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

      Object.entries(marketData.luxury).forEach(([type, data]) => {
  ...
  buyCar: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();

    if (!action || action === "list") {
      let carList = `${fonts.bold("🚗 LUXURY VEHICLES")}\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n\n`;

      Object.entries(marketData.vehicles).forEach(([type, data]) => {
        carList += `🚗 ${data.name}\n`;
  ...
  gamble: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
    if (!amount || amount <= 0) {
      return message.reply(fonts.bold(`
🎰 GAMBLING GAMES
...
Gambling Skill: ${bank.skills.gambling}
  ...
  slots: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
    if (!amount || amount <= 0) {
...
`;
  ...
  blackjack: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
    if (!amount || amount <= 0) {
...
`;
  ...
  roulette: async function (message, args, bank, fonts, userId, saveBank) {
    const amount = parseInt(args[1]);
...
`;
  ...
  lottery: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
  ...
  premium: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
...
`;
  ...
  vault: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
    const amount = parseInt(args[2]);
...
• bank vault withdraw <amount>
  ...
  insurance: async function (message, args, bank, fonts, userId, saveBank) {
    const action = args[1]?.toLowerCase();
...
`;
  ...
  leaderboard: async function (message, fonts, bot, chatId) {
    try {
      const bankDb = getDatabase('bank');
      const balances = getDatabase('balances');

      const users = [];
      for (const [uid, bank] of Object.entries(bankDb)) {
        if (bank && bank.balance + bank.savings + bank.vault > 0) {
          let name = `User ${uid}`;
          try {
            const member = await bot.getChatMember(chatId, parseInt(uid));
            name = member.user.first_name || member.user.username || name;
          } catch (e) {}
          users.push({
            uid,
            name,
  ...
  rob: async function (message, args, bank, fonts, userId, saveBank, bot, chatId, msg) {
    let targetUID = null;
    if (msg.entities) {
      for (const entity of msg.entities) {
    ...
    const bankDb = getDatabase('bank');
    let targetBank = bankDb[targetUID];
    if (!targetBank) {
      targetBank = createDefaultBank(targetUID);
    }
    ...
    if (success) {
      ...
      targetBank.transactions.push({
        ...
        amount: stolenAmount,
        date: Date.now(),
        description: `Robbed by user ${userId}`
      });

      bankDb[userId] = bank;
      bankDb[targetUID] = targetBank;
      saveDatabase('bank', bankDb);
  ...
  onStart: async function ({ bot, message, msg, chatId, args }) {
    ...
    let bankDb = getDatabase('bank');
    if (!bankDb[userId]) {
      bankDb[userId] = createDefaultBank(userId);
      saveDatabase('bank', bankDb);
    }
    const bank = bankDb[userId];

    const command = args[0]?.toLowerCase();

    // Fonctions de sauvegarde
    const saveBalances = () => saveDatabase('balances', balances);
    const saveBank = () => {
      bankDb[userId] = bank;
      saveDatabase('bank', bankDb);
    };
    ...
    if (bank.frozen && !["help", "balance", "history", "freeze", "leaderboard", "achievements", "credit"].includes(command)) {
      return message.reply(fonts.bold("🔒 Your account is frozen. You cannot perform transactions. Use 'bank freeze' to unfreeze."));
    }
    ...
    switch (command) {
      ...
      case "balance":
      case "bal":
        return this.showBalance(message, bank, balances[userId].money || 0, fonts);

      case "deposit":
      case "dep":
        return this.deposit(message, args, balances[userId], bank, fonts, userId, balances, saveBalances, saveBank);

      case "withdraw":
      case "wd":
        return this.withdraw(message, args, balances[userId], bank, fonts, userId, balances, saveBalances, saveBank);

      case "transfer":
      case "send":
        return this.transfer(message, args, balances[userId], bank, fonts, userId, balances, saveBalances, saveBank, bot, chatId, msg);

      case "loan":
        return this.loan(message, args, bank, fonts, userId, saveBank);

      case "repay":
        return this.repayLoan(message, args, bank, fonts, userId, saveBank);

      case "savings":
      case "save":
        return this.savings(message, args, bank, fonts, userId, saveBank);

      case "interest":
        return this.calculateInterest(message, bank, fonts, userId, saveBank);

      case "collect":
        return this.collectInterest(message, bank, fonts, userId, saveBank);

      case "history":
        return this.showHistory(message, bank, fonts);

      case "freeze":
        return this.freezeAccount(message, bank, fonts, userId, saveBank);

      case "daily":
        return this.dailyReward(message, bank, fonts, userId, saveBank);

      case "work":
        return this.work(message, bank, fonts, userId, saveBank);

      case "invest":
        return this.invest(message, fonts);

      case "stocks":
        return this.stocks(message, args, bank, fonts, userId, saveBank);

      case "bonds":
        return this.bonds(message, args, bank, fonts, userId, saveBank);

      case "crypto":
        return this.crypto(message, args, bank, fonts, userId, saveBank);
      ...
      case "dividend":
        return this.collectDividend(message, bank, fonts, userId, saveBank);

      case "business":
        return this.business(message, args, bank, fonts, userId, saveBank);

      case "shop":
        return this.shop(message, args, bank, fonts, userId, saveBank);
      ...
      case "house":
        return this.buyHouse(message, args, bank, fonts, userId, saveBank);

      case "rent":
        return this.rentProperty(message, bank, fonts, userId, saveBank);

      case "luxury":
        return this.luxury(message, args, bank, fonts, userId, saveBank);

      case "car":
        return this.buyCar(message, args, bank, fonts, userId, saveBank);

      case "gamble":
        return this.gamble(message, args, bank, fonts, userId, saveBank);

      case "lottery":
        return this.lottery(message, args, bank, fonts, userId, saveBank);

      case "slots":
        return this.slots(message, args, bank, fonts, userId, saveBank);

      case "blackjack":
        return this.blackjack(message, args, bank, fonts, userId, saveBank);

      case "roulette":
        return this.roulette(message, args, bank, fonts, userId, saveBank);

      case "premium":
        return this.premium(message, args, bank, fonts, userId, saveBank);

      case "vault":
        return this.vault(message, args, bank, fonts, userId, saveBank);

      case "insurance":
        return this.insurance(message, args, bank, fonts, userId, saveBank);
      ...
      case "leaderboard":
      case "lb":
        return this.leaderboard(message, fonts, bot, chatId);

      case "rob":
        return this.rob(message, args, bank, fonts, userId, saveBank, bot, chatId, msg);
...
module.exports = { onStart: nix.onStart, nix };