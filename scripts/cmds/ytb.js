...
async function getAPIBase() {
  try {
    ...
    if (!data?.api) throw new Error("API manquante");
    return data.api;
  } catch (error) {
...
async function downloadMedia(videoUrl, type, apiBase, chatId, bot, msg, fileName) {
  try {
    ...
    if (!data?.status || !downloadUrl) throw new Error("Erreur API");
